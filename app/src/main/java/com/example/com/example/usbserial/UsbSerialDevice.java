...
import com.example.deviceIds.CH34xIds;
import com.example.deviceIds.CP210xIds;
import com.example.deviceIds.FTDISioIds;
import com.example.deviceIds.PL2303Ids;
...
public abstract class UsbSerialDevice implements UsbSerialInterface {
    ...
    public static UsbEndpoint inEndpoint;
    public static UsbEndpoint outEndpoint;
    ...
    public int syncWrite(byte[] buffer, int timeout) {
        System.out.println("asyncmode : " + asyncMode);

        if (!asyncMode) {

            if (buffer == null)
                return 0;
            System.out.println("outend : " + outEndpoint);

            String data = new String(buffer);
            Log.d(TAG, "converted data into String : " + data);
            // vusb.printText(data,outEndpoint,inEndpoint);
    ...
    public int syncRead(byte[] buffer, int timeout) {
        System.out.println("asyncmode 1: " + asyncMode);
        if (asyncMode) {
        ...
        System.out.println("inEndpoint" + inEndpoint + "buffer" + buffer + "buffer.length" + buffer.length + "timeout" + timeout);
    ...
    public byte[] readData() {
        ...
        while (true) {
            int bytesCount = connection.bulkTransfer(inEndpoint, mReadData, mReadData.length, 0);
