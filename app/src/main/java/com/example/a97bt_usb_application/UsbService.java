...
import com.example.com.example.usbserial.CDCSerialDevice;
import com.example.com.example.usbserial.UsbSerialDevice;
import com.example.com.example.usbserial.UsbSerialInterface;
        ...
        public void onReceive(Context arg0, Intent arg1) {
            if (arg1.getAction().equals(ACTION_USB_PERMISSION)) {
                ...
                if (granted) // User accepted our USB connection. Try to open the device as a serial port
                {
                    ...
                    vusb.usbCommunication=connection;
                    System.out.println("connection value : " + vusb.usbCommunication);
//                    connection.controlTransfer(0x21, 0x22, 0x1, 0, null, 0, 0);
    ...
    public void write(byte[] data) {
        Log.d(TAG,"serial port check");
        if (serialPort != null) {
            Log.d(TAG,"serial port not null");
            serialPort.syncWrite(data, 0);
        } else {
            Log.d(TAG,"serial port is null");

            Toast.makeText(getApplicationContext(), "serial port is null", Toast.LENGTH_SHORT);
        }
    }

    public int read(byte[] data) {
        if (serialPort != null) {
            Log.d("VISIONTEK ", "serial port not null");


            return serialPort.syncRead(data, 10000);
        } else {
            Toast.makeText(getApplicationContext(), "serial port is null", Toast.LENGTH_SHORT);
            return -1;
        }
...
//                int n = serialPort.syncRead(buffer, 1000);
//                if (n > 0) {
//                    byte[] received = new byte[n];
//                    System.out.println("received bytes : " + Arrays.toString(received));
